...
export class STARKHDKey {
    ...
    public set privateKey(value: string | BN) {
        if (typeof value === 'string') {
            ...
            if (valueNum.gt(STARK.n) || valueNum.eq(0)) throw 'Invalid private key'
            this._privateKey = valueNum
        } else {
            if (value.gt(STARK.n) || value.eq(0)) throw 'Invalid private key'
            this._privateKey = value
        }

        this._publicKey = STARK.g.mul(this._privateKey)
        this._identifier = STARKHDKey.hash160(cryptoJS.enc.Hex.parse(this._publicKey.encodeCompressed('hex')))
    ...
    public set publicKey(value: any) {
        if(typeof value === 'string'){
            ...
            this._publicKey = STARK.curve.decodePoint(value, 'hex')
        }else {
            if(!STARK.curve.validate(value)) throw 'Invalid public key'
    ...
    public get xprv(): string{
        if (this._privateKey) {
            ...
            return bs58check.encode(STARKHDKey.serialize(this, this.versions.private, keyBuff))
        }
        else return null
    }

    public get xpub(): string{
        return bs58check.encode(STARKHDKey.serialize(this, this.versions.public, this._publicKey.encodeCompressed('hex')))
    }

    public derive(path: string): STARKHDKey{
    ...
    public deriveChild(index: number): STARKHDKey {
        ...
        let hd = new STARKHDKey(this.versions)
    ...
    public publicDerive(path: string): [STARKHDKey, BN] {
        let delta = ZERO
        if (path === 'm' || path === 'M' || path === "m'" || path === "M'") {
        ...
        //   let hardened = (c.length > 1) && (c[c.length - 1] === "'")
            ...
            //    if (hardened) childIndex += HARDENED_OFFSET
                ...
                delta = delta.add(_delta).umod(STARK.n)
                // @ts-ignore
                hdkey = _hdkey
            } else {
                throw "Invalid index"
    ...
    public publicDeriveChild(index: number): [STARKHDKey, BN]{
        //let isHardened = index >= HARDENED_OFFSET
    ...
    public static fromMasterSeed(seedWordArray: CryptoJSBytes): STARKHDKey {
        ...
        let hdkey = new STARKHDKey(BITCOIN_VERSIONS)
        hdkey.privateKey = IL
        hdkey.chainCode = IR
    ...
    public static fromMasterSeedHex(seedHex: string): STARKHDKey {
        return STARKHDKey.fromMasterSeed(cryptoJS.enc.Hex.parse(seedHex))
    }

    public static fromPublicKeyAndChainCode(publicKey: any, chainCode: any): STARKHDKey {
        let hdkey = new STARKHDKey(BITCOIN_VERSIONS)
    ...
    public static fromPrivateKeyAndChainCode(privateKey: BN, chainCode: any): STARKHDKey{
        let hdkey = new STARKHDKey(BITCOIN_VERSIONS)
    ...
    public static fromExtendedKey(base58key: string) {
        ...
        let hdkey = new STARKHDKey(versions)
        ...
        if(prefix === 0){ // private
            ...
            hdkey.publicKey =  STARK.curve.decodePoint(keyBufferHex.substr(45 * 2, 33 * 2), 'hex')
        }
        return hdkey
    }
