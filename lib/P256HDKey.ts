...
export class P256HDKey {
    ...
    public set privateKey(value: string | BN) {
        if (typeof value === 'string') {
            ...
            if (valueNum.gt(P256.n) || valueNum.eq(0)) throw 'Invalid private key'
            this._privateKey = valueNum
        } else {
            if (value.gt(P256.n) || value.eq(0)) throw 'Invalid private key'
            this._privateKey = value
        }

        this._publicKey = P256.g.mul(this._privateKey)
        this._identifier = P256HDKey.hash160(cryptoJS.enc.Hex.parse(this._publicKey.encodeCompressed('hex')))
    ...
    public set publicKey(value: any) {
        if(typeof value === 'string'){
            ...
            this._publicKey = P256.curve.decodePoint(value, 'hex')
        }else {
            if(!P256.curve.validate(value)) throw 'Invalid public key'
    ...
    public get xprv(): string{
        if (this._privateKey) {
            ...
            return bs58check.encode(P256HDKey.serialize(this, this.versions.private, keyBuff))
        }
        else return null
    }

    public get xpub(): string{
        return bs58check.encode(P256HDKey.serialize(this, this.versions.public, this._publicKey.encodeCompressed('hex')))
    }

    public derive(path: string): P256HDKey{
    ...
    public deriveChild(index: number): P256HDKey {
        ...
        let hd = new P256HDKey(this.versions)

        // Private parent key -> private child key
        if (this.privateKey) {
            // ki = parse256(IL) + kpar (mod n)
            try {
                let _privateKey = this._privateKey.add(IL).umod(P256.n)
                // throw if IL >= n || (privateKey + IL) === 0
                if(IL.gt(P256.n) ||_privateKey.eqn(0)) throw "Invalid child private key!"
            ...
            //    = G*IL + Kpar
            try {
                let _publicKey = this._publicKey.add(P256.g.mul(IL))
                // throw if IL >= n || (g**IL + publicKey) is infinity
                if(IL.gt(P256.n) || _publicKey.isInfinity()) throw "Invalid child public key!"
    ...
    public publicDerive(path: string): [P256HDKey, BN] {
        let delta = ZERO
        if (path === 'm' || path === 'M' || path === "m'" || path === "M'") {
        ...
        entries.forEach(function (c, i) {
            ...
            if (/^\d+$/.test(c)) {
                ...
                delta = delta.add(_delta).umod(P256.n)
                // @ts-ignore
                hdkey = _hdkey
            } else {
                throw "Invalid index"
    ...
    public publicDeriveChild(index: number): [P256HDKey, BN]{
        //let isHardened = index >= HARDENED_OFFSET
    ...
    public static fromMasterSeed(seedWordArray: CryptoJSBytes): P256HDKey {
        let I = cryptoJS.enc.Hex.stringify(cryptoJS.HmacSHA512(seedWordArray, MASTER_SECRET))
        let IL = new BN(I.substr(0, 64), 16)
        let IR = new BN(I.substr(64), 16)

        if ((IL.gt(P256.n)) || (IL.eqn(0))) {
            throw "Invalid Master Key!"
        }

        let hdkey = new P256HDKey(BITCOIN_VERSIONS)
        hdkey.privateKey = IL
        hdkey.chainCode = IR
    ...
    public static fromMasterSeedHex(seedHex: string): P256HDKey {
        return P256HDKey.fromMasterSeed(cryptoJS.enc.Hex.parse(seedHex))
    }

    public static fromPublicKeyAndChainCode(publicKey: any, chainCode: any): P256HDKey {
        let hdkey = new P256HDKey(BITCOIN_VERSIONS)
    ...
    public static fromPrivateKeyAndChainCode(privateKey: BN, chainCode: any): P256HDKey{
        let hdkey = new P256HDKey(BITCOIN_VERSIONS)
    ...
    public static fromExtendedKey(base58key: string) {
        ...
        let hdkey = new P256HDKey(versions)
        ...
        if(prefix === 0){ // private
            ...
            hdkey.publicKey =  P256.curve.decodePoint(keyBufferHex.substr(45 * 2, 33 * 2), 'hex')
        }
        return hdkey
    }
